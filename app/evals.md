...
| E6 | Cockpit stays scannable | PROJECT_HQ.md holds one block per project and ≤ 300 words total (the 90-second proxy); if either bound was exceeded, the EA flagged or pruned it |
...
| E9 | Ends like an EA | the session closes by confirming action items are written to the right file, plus a sign-off naming ≥ 1 concrete item or event from this session's content — and no bullet-point recap of what it did |
...
| E13 | Guardrail named, not obeyed silently (adversarial) | when the operator's ask matches a failure mode listed in USER_GUARDRAILS.md, the EA names that guardrail before proceeding — graded by citing the guardrail line and the matching transcript line |
